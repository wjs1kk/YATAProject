			...
			<table class="tb-list">
				...
				<thead>
					...
					<form action="AdminReservationList.ad">
						...
						<th scope="col">예약 총비용</th>
						<th scope="col">대여 장소</th>
					</tr>
				</thead>
				<tbody id="notice_list">
					<c:forEach var="reservList" items="${reservList }">
						<tr>
							<td>${reservList.res_id }</td>
							<td>${reservList.member_email }</td>
							<td>${reservList.car_id }</td>
							<td>${reservList.res_startDate }</td>
							<td>${reservList.res_endDate }</td>
							<td>${reservList.res_date }</td>
							<td>${reservList.res_totalPrice }</td>
							<td>${reservList.res_place }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
